...
Run `node scripts/sync-upstream.js`. It fetches the submodule's upstream `main`, reports whether the
...
1. Read the diff. For each changed *guidance* item, check §3 (overrides of Microsoft) in `skills/semantic-model/SKILL.md` and `skills/report/SKILL.md` — does an
   existing override still make sense? Does a new MS recommendation contradict a firm rule?
2. Run `evals/run.js` against the bumped submodule.
