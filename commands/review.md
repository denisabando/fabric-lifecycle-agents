...
If `$ARGUMENTS` ends in `.Report`, delegate to `report-reviewer`; otherwise delegate to the `reviewer` subagent with the model path `$ARGUMENTS`. It writes `clients/<code>/reviews/<Model>-<yyyymmdd>.md`. Summarise the verdict and the top
