...
| Firm | `skills/standards/` — `modeling.md`, `dax.md`; `overrides.md` for departures from Microsoft (+ `rules/` for the machine-checkable subset) | naming, structure, DAX style, review gates |
...
conflict in `skills/standards/overrides.md` (date, rule, which MS guidance it overrides, why) so it is
